...
@Component({
  selector: 'app-diagonal-record-list',
  templateUrl: './diagonal-record-list.component.html',
  styles: [
  ]
})
export class DiagonalRecordListComponent extends ReportListComponent implements OnInit {
  schema: SFSchema = {
    properties: {
      deviceRecordUuid: {
        type: 'string',
  ...
  constructor(public deviceRecordService: DeviceRecordService, public diagonalRecordService: DiagonalRecordService, public router: Router,
    ...
    super(servicePathService.diagonalRecord, diagonalRecordService, modal, cdr, router, deviceRecordService );
  ...
  deviceRecordProcess(successData, failureData) {
    super.deviceRecordProcess(this.schema, this.columns, successData, failureData, 'diagonalRecord', this.diagonalRecordService.listPropertys);
  }
  ngOnInit(): void {
  }
