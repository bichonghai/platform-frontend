...
@Component({
  selector: 'app-height-gauge-record-list',
  templateUrl: './height-gauge-record-list.component.html',
  styles: [
  ]
})
export class HeightGaugeRecordListComponent extends ReportListComponent implements OnInit {
  schema: SFSchema = {
    properties: {
      deviceRecordUuid: {
        type: 'string',
  ...
  constructor(public deviceRecordService: DeviceRecordService, public heightGaugeRecordService: HeightGaugeRecordService, public router: Router,
    ...
    super(servicePathService.heightGaugeRecord, heightGaugeRecordService, modal, cdr, router, deviceRecordService );
  ...
  deviceRecordProcess(successData, failureData) {
    super.deviceRecordProcess(this.schema, this.columns, successData, failureData, 'heightGaugeRecord', this.heightGaugeRecordService.listPropertys);
  }
  ngOnInit(): void {
  }
